...
namespace Thread::NiNode::Surface::Geometry
{
    ...
    ActorGeometry::ActorGeometry(RE::Actor* a_actor)
    {
        ...
        const auto getNode = [&](std::string_view a_name, auto& a_target, bool a_log) {
            ...
            if (!node) {
                if (a_log) {
                    logger::info("Actor {:X} (Race: {}) is missing node {} (this may be expected)", a_actor->GetFormID(), racestr, a_name);
                }
                return false;
            }
        ...
        logger::info("Surface collision shaft discovery actor {:X}: configured bases found={}", a_actor->GetFormID(), shafts.size());
        ...
        if (vaginalGeometry) {
            ...
            logger::info("Surface collision vaginal surface selected '{}' in '{}': max weights left={:.4f} ({} vertices), right={:.4f} ({} vertices)", vaginalGeometry->name,
        ...
        if (analGeometry) {
            ...
            logger::info("Surface collision anal surface selected '{}' in '{}': max weights left={:.4f} ({} vertices), right={:.4f} ({} vertices)", analGeometry->name,
        ...
        if (mouth) {
            ...
            logger::info("Actor {:X} mouth surface selected '{}' under '{}': center=({:.2f}, {:.2f}, {:.2f}), radius={:.2f}, opening={}", a_actor->GetFormID(), mouth->name,
                mouth->parent ? mouth->parent->name.c_str() : "", center.x, center.y, center.z, mouth->worldBound.radius, mouthOpening.has_value());
        }
        logger::info("Actor {:X} surface shapes: mouth={}, vaginal={}, anal={}", a_actor->GetFormID(), mouthOpening.has_value(), trackedVagina.has_value(), trackedAnus.has_value());
    }
