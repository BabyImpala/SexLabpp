...
namespace Thread::NiNode::Surface::Geometry
{
            ...
            default:
                {
                    ...
                    if (direction.SqrLength() > FLT_EPSILON) {
                        ...
                        logger::error("Ambiguous skeleton structure for user {:X} at node depth {}", user ? user->GetFormID() : 0, skeletonNodes.size());
                    }
                }
                break;
            }
    ...
    void Shaft::DiscoverSurface(RE::Actor* a_actor)
    {
        ...
        if (!bestGeometry) {
            logger::info("Surface collision shaft surface not found for actor {:X}: base='{}', geometries={}, skinned={}, longest weighted descendant chain={}", a_actor->GetFormID(),
        ...
        logger::info("Surface collision shaft surface selected '{}' in '{}': base='{}', skinned chain bones={}", bestGeometry->name, modelPath, baseName, bestWeights.chainBones.size());
        if (BindSurface(bestGeometry.get(), modelPath, std::addressof(bestWeights.chainBones))) {
            ...
            logger::info("Surface collision shaft surface bind failed for '{}' in '{}': base='{}'", bestGeometry->name, modelPath, baseName);
        }
    }
    ...
    bool Shaft::BindSurface(RE::BSGeometry* a_geometry, std::string_view a_modelPath, const std::vector<std::uint16_t>* a_knownChain)
    {
        ...
        logger::info("Surface collision shaft surface topology {} for '{}' in '{}' ({:016X}, {} rings)", cacheHit ? "cache hit" : "cached", a_geometry->name, a_modelPath,
    ...
    void Shaft::UpdateCollisionShape()
    {
        ...
        if (!surface) {
            ...
            if (surfaceSearchPending && ++stableEquipmentFrames >= SHAFT_EQUIPMENT_STABLE_FRAMES) {
                ...
                logger::info("Surface collision shaft delayed surface initialization: {:.2f}ms ({})", elapsed.count(), surface ? "bound" : "not found");
            }
            if (!surface) {
                collisionShape.reset();
                return;
