...
{
    using NiType = Thread::Interaction::NiML::NiType::Type;
    ...
    std::string MLStart(STATICARGS, std::string a_type, bool a_enabled)
    {
        ...
        Thread::Interaction::NiML::NiUpdate::UpdateMLTrainingState(type, a_enabled);
    ...
    {
        const auto state = Thread::Interaction::NiML::NiUpdate::GetMLTrainingState();
        if (a_state == -1)
            a_state = 1 - state.enabled;
        if (a_state != 0 && a_state != 1) {
            return "Invalid state. Valid states are: 0 (disable), 1 (enable), -1 (toggle)";
        }
        Thread::Interaction::NiML::NiUpdate::UpdateMLTrainingState(state.type, a_state == 1);
    ...
    {
        Thread::Interaction::NiML::NiUpdate::UpdateMLTrainingState(NiType::None, false);
    ...
    std::string MLSetInterval(STATICARGS, int a_frameInterval)
    {
        ...
        Thread::Interaction::NiML::NiUpdate::SetMLTrainingFrameInterval(static_cast<size_t>(a_frameInterval));
        return std::format("ML training frame interval set to {} frames", a_frameInterval);
    }

    std::string MLDrop(STATICARGS)
    {
        Thread::Interaction::NiML::NiUpdate::ClearMLTrainingData();
