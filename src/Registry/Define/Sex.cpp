		...
		if (!a_skipfactions) {
			a_actor->VisitFactions([&](auto a_faction, auto a_rank) {
				if (a_faction == GameForms::GenderFaction) {
					logger::info("[GetSex] Actor {:X} GenderFaction rank {}", a_actor->GetFormID(), a_rank);
					switch (a_rank) {
					case 0: ret = Sex::Male; break;
					case 1: ret = Sex::Female; break;
					case 2: ret = Sex::Futa; break;
					default: logger::info("Actor {:X} has invalid gender faction rank ({})", a_actor->GetFormID(), a_rank); break;
					}
					return true;
				}
				return false;
